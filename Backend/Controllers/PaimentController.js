...
exports.UpdatePaiment = async (req, res) => {
  try {
    const { body } = req;
    if ((await Paiment.updateOne({ _id: req.params.id }, { ...body })).modifiedCount)
...
// method : get => url : /api/paiment/show_paiment =>acces : Private
  exports.ShowPaiment=async(_,res)=>{
    try{
      res.status(200).json(await Paiment.find().populate({path:'appartementid', model: Appratemen , populate: { path: "client_id",model:Client }}))
    }catch(e){
      res.status(400).send(e);
    }
   }